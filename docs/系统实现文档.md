...
实现了对于各个表的增、删、改、查基本操作。查询以用户登录操作为例，首先接收到前端以JSON格式传入的邮箱和密码信息，接着在 `User` 表中以邮箱为关键词进行查询，若存在对应的用户，则进一步检查用户的密码和传入的密码是否相同，若密码相同则返回登录成功，否则返回密码错误。若不存在相应的用户则返回该用户不存在。
